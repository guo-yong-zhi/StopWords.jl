...
struct StopWordsDict <: AbstractDict{Any, Set{String}}
    ...
    dict2::Dict{Vector{String}, Set{String}}
    languages::Set{String}
    path::String
end
function StopWordsDict(path=DATA_PATH)
    languages = Set([f[1:end-4] for f in readdir(path)])
    return StopWordsDict(Dict{String, Set{String}}(), Dict{Vector{String}, Set{String}}(),languages, path)
end
...
function Base.getindex(sw::StopWordsDict, langs::Vector{String})
...
"""
    ...
    haskey(sw::StopWordsDict, langs::Vector{String}) -> Bool
...
Base.haskey(sw::StopWordsDict, langs::Vector{String}) = all(haskey(sw, lang) for lang in langs)
...
And to get the stop words for multiple languages, you can use a vector, e.g. `stopwords[["en", "fr"]]`.
"""
